...
function createFromRaw (raw, callback) {
  ...
  mailParser(raw, function (err, parsedEmail) {
    var msg = new Message()
    if (parsedEmail) {
      msg.messageId = parsedEmail.messageId || ''

      msg.from = parsedEmail.from ? parsedEmail.from.value[0] : null
      ...
      parsedAttachments.forEach(function (parsedAttachment) {
        var attach = new Attachment(
          parsedAttachment.generatedFileName,
      ...
      msg.to = parsedEmail.to ? parsedEmail.to.value : []
      msg.cc = parsedEmail.cc ? parsedEmail.cc.value : []
      msg.bcc = parsedEmail.bcc ? parsedEmail.bcc.value : []
    }
    msg.size = raw.length
    callback(null, msg)
  })
