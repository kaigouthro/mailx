...
ImapStore.prototype.getMailboxes = function(callback) {
	...
	for (mailbox in boxes) {
	    boxes[mailbox].noselect = boxes[mailbox].attribs.indexOf('\\NOSELECT') !== -1 ? true : false;
	}
...
ImapStore.prototype.getHeaders = function(box, start, callback) { 
    var self = this;
    self._openBox(box, function(err, box) {
	if (err) {
	    return callback(err);
	}
	self._fetchWithOptions('HEADER.FIELDS (FROM TO SUBJECT DATE)', start, callback);
    });
}
...
ImapStore.prototype._openBox = function(box, cb) {
    this.IMAPClient.openBox(box, false, cb);
}
...
ImapStore.prototype._fetchWithOptions = function(bodies, start, callback) {
  var self = this;
  var start = start || 1;
  var msgs = [];
  var fetchMessages = self.IMAPClient.fetch(start + ':*', {bodies: bodies, struct: true});
