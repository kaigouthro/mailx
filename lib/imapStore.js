...
ImapStore.prototype.getInboxMessagesAsStream = function(from, to, batchSize, callback) {
  var self = this;
  var lastBatch = false;
  ...
  var fetchNextBatch = function(start, end, cb) {
    ...
    var fetchMessages = self.IMAPClient.fetch(start + ':' + end, { bodies: '', struct: true });
    var msgs = [];
...
ImapStore.prototype.move = function(uids, boxTo, cb) {
  this.IMAPClient.move(uids, boxTo, cb);
};
...
ImapStore.prototype.getBox = function(boxName, cb) {
  var self = this;
  if (!this.boxes) {
    this.getBoxes(function(err, boxes) {
      if (err) {
        return cb(err);
      }
