...
PopStore.prototype.getMailboxes = function (callback) {
  var inbox = {
    INBOX: {
      attribs: [],
      delimiter: '/',
...
PopStore.prototype.getHeaders = function (box, start, callback) {
  var self = this
  self._iterate(start, callback, function (seqno, cb) {
    self.POP3Client.top(seqno, 0, function (err, rawMessage) {
      if (err) {
        return cb(err)
      }
...
PopStore.prototype.getMessages = function (box, start, callback) {
  this.getInboxMessages(start, callback)
}
...
PopStore.prototype._iterate = function (start, callback, iterator) {
  var self = this
  ...
  self._list(start, function (err, list) {
    ...
    var inbox = asynk.each(list, iterator).serie()
    inbox.fail(callback)
    inbox.done(function (messages) {
      var parsedMessages = asynk
        .each(messages, function (message, cb) {
